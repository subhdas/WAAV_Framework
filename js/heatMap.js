...
var colors = ['#FFFFFF','#F1EEF6','#E6D3E1','#DBB9CD','#D19EB9','#C684A4','#BB6990','#B14F7C','#A63467','#9B1A53','#91003F'];
...
function loadHeatMap() {
	console.log("Loading Heat Map Vis");
	d3.csv("data/armsData.csv", function(data) {
		row_number = data.length;
		columns = Object.keys(data[0]);
		col_number = columns.length;
		...
		for (var i = 0; i < row_number; i++) {
			if ($.inArray(data[i]["Reporter_Name"].trim(), rowLabel) > -1) {
				// already in the array
			} else {
				rowLabel.push(data[i]["Reporter_Name"].trim());
			}
			
			if ($.inArray(data[i]["Partner_Name"].trim(), rowLabel) > -1) {
				// already in the array
			} else {
				rowLabel.push(data[i]["Partner_Name"].trim());
			}
		}
		
		// initialize value matrix, populate with 0's
		valMtx = new Array(rowLabel.length); 
		for (var i = 0; i < rowLabel.length; i++) {
			valMtx[i] = new Array(rowLabel.length);
			for (var j = 0; j < valMtx[i].length; j++) {
				valMtx[i][j] = 0; 
			}
		}
		
		// populate value matrix with actual values
		for (var i = 0; i < row_number; i++) {
			var xInd = rowLabel.indexOf(data[i]["Reporter_Name"].trim());
			var yInd = rowLabel.indexOf(data[i]["Partner_Name"].trim());
			valMtx[xInd][yInd] = Number(valMtx[xInd][yInd]) + Number(data[i]["Value"]);
		}
		console.log(valMtx);
